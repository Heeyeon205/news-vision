...
VALUES ('AI 뉴스 대폭발', '지난해 윤석열 전 대통령이 투르크메니스탄 국빈 방문 때 선물 받은 국견(國犬) 알라바이 ''해피''와 ''조이''에 대한 사육비로 매년 약 670만원이 드는 것으로 파악됐다.
...
이미지 확대대통령 기록 전시관
대통령 기록 전시관
(세종=연합뉴스) 배재만 기자 = 윤석열 전 대통령 파면에 따라 현 정부의 대통령기록물 이관을 위한 현장 점검이 시작됐다. 10일 세종시 대통령기록관 내 대통령기록 전시관 모습. 2025.4.10 scoop@yna.co.kr
...
양 의원은 "국가 예산이 아닌 지자체 예산으로 대통령기록물이 관리되는 모순적인 상황이 발생하고 있다"며 "동물권을 고려하고 사회적 합의를 거쳐 생물인 대통령기록물을 관리하는 방안이 필요하다"고 지적했다.', 2, NOW(), 15, 1),
...
◇ 통상협의 "순조로운 첫 발"…상호관세 불확실성은 여전

27일 관계 당국에 따르면 한미 양국은 지난 25일 미국 워싱턴 DC에서 열린 2+2 통상 협의에서 이른바 ''7월 패키지'' 합의를 추진하기로 의견을 모았다.

미국의 90일 상호관세 유예가 종료되기 전에 7월 8일까지는 관세 폐지를 목표로 협상을 본격화하겠다는 것이다.

미국 측이 비관세 장벽 중 하나로 주장하는 ''환율'' 문제는 기획재정부와 미국 재무부 채널에서 별도 논의하기로 했다. 미국 도널드 트럼프 대통령이 집권 전부터 지적해 온 방위비 증액 문제도 테이블에 오르지 않았다.
...
이미 철강·알루미늄·자동차에 25%의 품목 관세에 더해 지난 5일부터는 10%의 기본 관세(보편관세)가 발효된 상황에서 25%의 상호관세는 한국 경제에 큰 추가 악재다. ''7월 패키지 딜''까지 상호 관세는 경제 주체들을 옥죄는 불확실성이 될 수 있다.
...
김광석 한국경제산업연구원 실장은 "지금 상호관세·조기 대선 등은 작년 예산안 편성 때 예측하지 못했던 불가항력의 변수들"이라며 "추가적인 추경 편성으로 경기 부양에 총력을 두어야 할 때"라고 말했다.', 3, NOW(), 22, 1),
...
아울러 여행 중 피해가 발생했을 땐 동영상이나 사진, 녹취자료 등의 입증 자료를 충분히 확보해 분쟁에 대비하라고 조언했다.', 4, NOW(), 8, 1),
...
경찰은 휴대전화 무단 개통 과정과 은행 거래 내용 등을 토대로 정확한 사건 경위를 조사 중인 것으로 전해졌다.', 5, NOW(), 10, 2),
